...
export function createOrchestratorRoutes(): Router {
  const router = Router();
  router.get("/", (_req, res) => res.json(ensureOrchestrator()));
  router.put("/", (req, res) => {
    const parsed = updateSchema.safeParse(req.body);
    if (!parsed.success) { res.status(400).json({ error: parsed.error.flatten() }); return; }
    const orchestrator = ensureOrchestrator();
    Object.assign(orchestrator, parsed.data, { role: "manager" as const, updatedAt: now() });
    markDirty();
    broadcastOrchestration({ id: createId(), projectId: "global", agentId: orchestrator.id, type: "orchestrator.updated", payload: orchestrator, sequence: 0, occurredAt: now() });
